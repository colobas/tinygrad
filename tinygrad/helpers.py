...
class tqdm(Generic[T]):
  ...
  def update(self, n:int=0, close:bool=False):
    ...
    # Long URLs must not consume the entire terminal width and hide the actual progress information.
    desc = self.desc[:max(ncols-len(suf)-7, 0)]
    sz = max(ncols-len(desc)-3-2-2-len(suf), 1)
    bar = '\r' + desc + (f'{100*prog:3.0f}%|{("█"*int(num:=sz*prog)+" ▏▎▍▌▋▊▉"[int(8*num)%8].strip()).ljust(sz," ")}| ' if self.t else '') + suf
