# reasoning-control Specification

## Purpose

Define how the LLM server parses, controls, and bounds model reasoning traces: splitting reasoning from visible content per-model, enforcing reasoning-effort levels and token budgets host-side, disabling thinking on request, and applying stop sequences to visible content only.
