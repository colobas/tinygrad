# chat-prompt-templating Specification

## Purpose

Define how the LLM server builds request prompts: rendering each model's native chat template over the request messages and tools, falling back to preset role formatting when no template is available, and handling prior-turn reasoning traces and assistant prefills correctly.
